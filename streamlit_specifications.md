...
# Streamlit Application Specification: Automation Project Evaluation

## 1. Application Overview

The **"Automation Project Evaluation: A CFA Institute Scorecard Approach for Investment Professionals"** Streamlit application serves as a systematic framework for evaluating and prioritizing potential AI/ML automation initiatives within an investment firm. Based on the persona of Alex Chen, CFA, an Investment Operations Manager at Prosperity Global Asset Management (PGAM), the app guides users through a structured workflow to move beyond ad-hoc decisions and identify high-value, feasible, and low-risk automation opportunities.

The application's high-level story flow is as follows:

1.  **Introduction & Scenario**: Alex is introduced to the problem of inconsistent automation results and the need for a structured evaluation framework.
2.  **Define Dimensions**: Users learn about and review the six key dimensions of the CFA Institute's Automation Scorecard, including the Multi-Criteria Decision Analysis (MCDA) formula.
3.  **Score Projects**: Users interactively score a pre-loaded set of candidate automation projects against these dimensions and can add their own custom tasks, seeing a heatmap visualization of scores.
4.  **Prioritize with Weights & Sensitivity**: Users apply different strategic weight profiles (e.g., 'Efficiency-First', 'Risk-First') to compute weighted suitability scores and observe how rankings change.
5.  **Classify Tiers**: Projects are classified into appropriate automation tiers (Traditional, GenAI/LLM, Human Intervention, Hybrid) based on rule-based logic.
6.  **Visualize Landscape**: Key insights are communicated through interactive visualizations, including a quadrant plot, suitability ranking chart, and comparative radar charts.
7.  **Deep Dive: Hybrid Routing & ROI**: For a selected top project, a conceptual hybrid routing strategy is presented, and a business case with ROI estimation is generated.
8.  **Prioritized Automation Roadmap**: All findings are consolidated into an actionable roadmap, categorizing projects into 'Quick Wins', 'Medium-Term', and 'Strategic' initiatives.

This application demonstrates a practical, hands-on approach for investment professionals to apply the CFA Institute's Automation Scorecard for strategic decision-making in AI/ML adoption.

---

## 2. Code Requirements

The Streamlit application will be implemented in `app.py`.
...
### 2.2 `st.session_state` Design

`st.session_state` will be used to maintain the application's state across user interactions and page navigation.

**Initialization (in `_initialize_session_state()` function):**

*   `st.session_state.current_page`: `str` - Stores the currently selected page from the sidebar navigation. Initialized to `"Introduction"`.
*   `st.session_state.scorecard_dimensions`: `dict` - Stores the `SCORECARD_DIMENSIONS` from `source.py`. Initialized once.
*   `st.session_state.score_columns`: `list` - Stores the `SCORE_COLUMNS` from `source.py`. Initialized once.
*   `st.session_state.project_scores_df`: `pd.DataFrame` - The core DataFrame containing project scores, suitability scores, tiers, and roadmap info.
    *   **Initialization**: Constructed by taking `data` from `source.py`, transposing it, and then concatenating the 'hardcoded' custom tasks (`custom_task_1_scores`, `custom_task_2_scores`) defined in `source.py` to match the notebook's initial state for calculations.
    *   `project_scores_df.index.name` will be `'Project_ID'`.
    *   `project_scores_df.columns.name` will be `'Dimension'`.
*   `st.session_state.weights_efficiency_first`: `dict` - Stores `weights_efficiency_first` from `source.py`. User-modifiable via sliders.
*   `st.session_state.weights_risk_first`: `dict` - Stores `weights_risk_first` from `source.py`. User-modifiable via sliders.
*   `st.session_state.selected_top_project_id`: `str` - Stores the `Project_ID` chosen for detailed ROI analysis. Initialized to the first project in `project_scores_df.index` or `None` if empty.
*   `st.session_state.roi_annual_hours_saved`: `int` - Stores the value for 'Annual Hours Saved' for ROI calculation. Initialized with example value (96) from `source.py`.
*   `st.session_state.roi_cost_per_hour`: `int` - Stores the value for 'Cost/Hour' for ROI calculation. Initialized with example value (150) from `source.py`.
*   `st.session_state.roi_development_cost`: `int` - Stores the value for 'Development Cost' for ROI calculation. Initialized with example value (1200) from `source.py`.
*   `st.session_state.roi_annual_operating_cost`: `int` - Stores the value for 'Annual Operating Cost' for ROI calculation. Initialized with example value (100) from `source.py`.
*   `st.session_state.tier_colors_map`: `dict` - Stores the `tier_colors_map` from `source.py`. Initialized once.
*   `st.session_state.hybrid_routing_df`: `pd.DataFrame` - Stores the `hybrid_routing_data` from `source.py` as a DataFrame. Initialized once.
*   `st.session_state.hours_saved_month_map_full`: `dict` - Stores the `hours_saved_month_map` from `source.py` and is dynamically extended for any user-added tasks.
*   `st.session_state.effort_map_full`: `dict` - Stores the `effort_map` from `source.py` and is dynamically extended for any user-added tasks.

**Update Logic:**

*   `st.session_state.project_scores_df`:
    *   Updated by `pd.concat` when a new custom task is added on the "Score Projects" page.
    *   Replaced by the edited DataFrame from `st.data_editor` on the "Score Projects" page.
    *   Columns `suit_efficiency`, `suit_risk`, `tier`, `color`, `hours_saved_month`, `effort`, `timeline` are updated by calling a helper function `_update_derived_columns()` whenever `project_scores_df` or weight profiles change.
*   `st.session_state.weights_efficiency_first`, `st.session_state.weights_risk_first`: Updated directly by `st.slider` widgets on the "Prioritize with Weights" page.
*   `st.session_state.selected_top_project_id`: Updated by `st.selectbox` on the "Deep Dive & ROI" page.
*   `st.session_state.roi_annual_hours_saved`, etc.: Updated by `st.number_input` widgets on the "Deep Dive & ROI" page.

### 2.3 UI Interactions and `source.py` Function Invocation

The application will use a sidebar selectbox to simulate a multi-page experience.

```python
# app.py structure
# ... imports and _initialize_session_state() ...

# Helper function to update derived columns (suitability, tier, color, roadmap info)
# This function will be called after any relevant state change (e.g., scores updated, weights changed)
def _update_derived_columns():
    ...
    # Initialize full maps with source.py data, then extend for custom tasks
...
_initialize_session_state() # Call once at app start
_update_derived_columns() # Ensure derived columns are up-to-date after initialization
...
### 2.4 Markdown and Page-Specific Logic
...
```python
    ...
    if not edited_df.equals(st.session_state.project_scores_df[st.session_state.score_columns]):
        # Update only the score columns, preserve other columns if they exist (suitability, tier, etc.)
        # Ensure that newly added rows in data_editor are handled correctly
        # This assumes data_editor returns a df with Project_ID as index.
        updated_df_scores_only = edited_df.copy()
        
        # Merge updated scores back into the session state DataFrame
        # First, ensure that new rows from edited_df are properly indexed if they came from 'add row'
        if len(updated_df_scores_only) > len(st.session_state.project_scores_df):
            # Identify new rows (they'll likely have default numeric indices if added manually in data_editor)
            new_rows = updated_df_scores_only.loc[~updated_df_scores_only.index.isin(st.session_state.project_scores_df.index)]
            # Give new rows a temporary ID for now, user can rename later, or require ID input above
            new_row_ids = [f"UserTask_{len(st.session_state.project_scores_df) + i + 1}" for i in range(len(new_rows))]
            new_rows.index = new_row_ids
            # Concatenate new rows to the existing df
            st.session_state.project_scores_df = pd.concat([st.session_state.project_scores_df, new_rows])
            st.session_state.project_scores_df = st.session_state.project_scores_df.combine_first(updated_df_scores_only) # Update existing scores

        else: # Only existing rows were edited
             st.session_state.project_scores_df.update(updated_df_scores_only)

        _update_derived_columns()
        st.rerun() # Rerun to apply changes and re-render
    ...
    with st.form("new_task_form"):
        ...
        if submitted:
            if new_task_id and new_task_id not in st.session_state.project_scores_df.index:
                ...
                # Note: The original source.py add_custom_task also added a 'Description' column,
                # but then noted 'scores_df will remain scores only'. We'll follow that by not adding it to scores_df for calculations.
                # However, the description is useful for context.
                # For this Streamlit blueprint, we'll store the description for later use if needed.
                # For now, simply add to scores_df.
                st.session_state.project_scores_df = pd.concat([st.session_state.project_scores_df, new_task_df])
                st.session_state.hours_saved_month_map_full[new_task_id] = 7 # Default for new tasks
                st.session_state.effort_map_full[new_task_id] = 'Med' # Default for new tasks
                _update_derived_columns()
    ...
    plt.close(fig_heatmap) # Close figure to prevent displaying multiple times
...
#### D. Page: 3. Prioritize with Weights
...
```python
    ...
    with col1:
        ...
        if current_weights_eff != st.session_state.weights_efficiency_first:
            ...
            _update_derived_columns() # Recalculate suitability scores
            st.rerun()
...
```python
    ...
    # Classify projects (already done by _update_derived_columns, just display)
