...
# Page Config
st.set_page_config(
    page_title="QuLab: Lab 12: Automation Project Evaluation", layout="wide")
...
def _update_derived_columns():
    ...
    df['suit_efficiency'] = df.apply(
        calculate_weighted_score, axis=1, weights=st.session_state.weights_efficiency_first)
    df['suit_risk'] = df.apply(
        calculate_weighted_score, axis=1, weights=st.session_state.weights_risk_first)
...
def _initialize_session_state():
    if 'initialized' not in st.session_state:
        ...
        if not st.session_state.project_scores_df.empty:
            st.session_state.selected_top_project_id = st.session_state.project_scores_df.index[
                0]
        else:
...
if st.session_state.current_page == "Introduction":
    ...
    st.markdown(
        f"## Case Study: Systematically Prioritizing AI/ML Projects at Prosperity Global Asset Management")
    st.markdown(
        f"**Persona:** Alex Chen, CFA, Investment Operations Manager at Prosperity Global Asset Management.")
    st.markdown(
        f"**Organization:** Prosperity Global Asset Management (PGAM), a mid-sized asset management firm.")
    ...
    st.markdown(
        f"3. **Prioritize with Weights**: Apply strategic weighting profiles.")
    st.markdown(
        f"4. **Classify Tiers**: Assign projects to automation categories.")
    st.markdown(
        f"5. **Visualize Landscape**: Gain insights through interactive plots.")
    st.markdown(
        f"6. **Deep Dive & ROI**: Explore hybrid solutions and financial justification.")
    st.markdown(
        f"7. **Automation Roadmap**: Generate a prioritized implementation plan.")
...
# Page: 1. Define Dimensions
    ...
    st.markdown(
        f"The weighted suitability score will be calculated using the Multi-Criteria Decision Analysis (MCDA) formula:")
    st.markdown(r"""$$
S_j = \sum_{d=1}^{D} W_d \cdot X_{jd}
$$
""")
    st.markdown(
        r"where $S_j$ is the suitability score for project $j$, $W_d$ is the weight assigned to dimension $d$, and $X_{jd}$ is the raw score of project $j$ on dimension $d$. A positive weight means a higher score on this dimension contributes positively to suitability; a negative weight means a higher score contributes negatively. $D$ is the total number of dimensions.")

    st.markdown(f"---")
    st.markdown(f"### CFA Automation Scorecard Dimensions")
    ...
    st.table(pd.DataFrame(st.session_state.scorecard_dimensions.items(),
...
# Page: 2. Score Projects
    ...
    if not edited_df.equals(st.session_state.project_scores_df[st.session_state.score_columns]):
        ...
        if len(updated_df_scores_only) > len(st.session_state.project_scores_df):
            # Identify new rows
            new_rows = updated_df_scores_only.loc[~updated_df_scores_only.index.isin(
                st.session_state.project_scores_df.index)]
            # Give new rows a temporary ID
            new_row_ids = [
                f"UserTask_{len(st.session_state.project_scores_df) + i + 1}" for i in range(len(new_rows))]
            new_rows.index = new_row_ids
            # Concatenate
            st.session_state.project_scores_df = pd.concat(
                [st.session_state.project_scores_df, new_rows])
            st.session_state.project_scores_df = st.session_state.project_scores_df.combine_first(
                updated_df_scores_only)
        else:
    ...
    with st.form("new_task_form"):
        new_task_id = st.text_input(
            "Project ID (e.g., 'T11: My New Task')", max_chars=50)
        ...
        for dim, desc in st.session_state.scorecard_dimensions.items():
            if dim == 'human_oversight':
                new_task_scores[dim] = st.radio(
                    f"**{dim.replace('_', ' ').title()}** ({desc})", options=[0, 1], key=f"new_score_{dim}")
            else:
                new_task_scores[dim] = st.slider(
                    f"**{dim.replace('_', ' ').title()}** ({desc})", min_value=1, max_value=5, value=3, key=f"new_score_{dim}")

        submitted = st.form_submit_button("Add Project")
        if submitted:
            if new_task_id and new_task_id not in st.session_state.project_scores_df.index:
                new_task_df = pd.DataFrame([new_task_scores], index=[
                                           new_task_id], columns=st.session_state.score_columns)
                st.session_state.project_scores_df = pd.concat(
                    [st.session_state.project_scores_df, new_task_df])
                # Default
                st.session_state.hours_saved_month_map_full[new_task_id] = 7
                # Default
                st.session_state.effort_map_full[new_task_id] = 'Med'
                _update_derived_columns()
                st.success(f"Project '{new_task_id}' added!")
                st.rerun()
            elif new_task_id in st.session_state.project_scores_df.index:
                st.error(
                    f"Error: Project ID '{new_task_id}' already exists. Please choose a different one.")
            else:
    ...
    st.markdown(
        f"This heatmap visually represents the scores of all projects across the defined dimensions.")

    fig_heatmap, ax_heatmap = plt.subplots(
        figsize=(14, max(8, len(st.session_state.project_scores_df) * 0.7)))
    sns.heatmap(st.session_state.project_scores_df[st.session_state.score_columns], annot=True,
                fmt='.0f', cmap='viridis', linewidths=.5, linecolor='lightgray', ax=ax_heatmap)
...
# Page: 3. Prioritize with Weights
    ...
    st.title(
        "3. Prioritizing with Weighted Suitability Scores (MCDA) and Sensitivity Analysis")
    ...
    st.markdown(
        f"The formula for calculating the weighted suitability score, $S_j = \sum_{{d=1}}^{{D}} W_d \cdot X_{{jd}}$, allows for flexibility.")
    st.markdown(
        r"where $S_j$ is the suitability score for project $j$, $W_d$ is the weight assigned to dimension $d$, and $X_{jd}$ is the raw score of project $j$ on dimension $d$. A higher positive weight on 'efficiency_impact', for instance, means tasks with high efficiency potential will rank higher under an 'Efficiency-First' profile. Conversely, a higher negative weight on 'risk_level' means high-risk tasks will be penalized more heavily under a 'Risk-First' profile.")
    ...
    st.markdown(
        f"Below are the calculated suitability scores based on the configured weight profiles.")
...
# Page: 4. Classify Tiers
    ...
    st.markdown(
        f"1.  **Traditional Automation:** Rules-based, structured, objective tasks.")
    st.markdown(
        f"2.  **GenAI / LLM Automation:** Tasks involving unstructured data, context, and generation.")
    st.markdown(
        f"3.  **Human Intervention Required:** High-risk or highly subjective tasks requiring human oversight.")
    ...
    st.markdown(
        f"*   **Traditional Automation** if `Task_Complexity <= 2` AND `Data_Structure <= 2` AND `Output_Objectivity <= 2`.")
    st.markdown(
        f"*   **Human Intervention Required** if `Risk_Level >= 4` OR (`Output_Objectivity >= 5` AND `Task_Complexity >= 5`).")
    st.markdown(
        f"*   **GenAI / LLM Automation** if `Data_Structure >= 4` OR `Task_Complexity >= 3`.")
    ...
    st.dataframe(st.session_state.project_scores_df['tier'].value_counts().reset_index(
    ).rename(columns={'index': 'Tier', 'tier': 'Count'}), use_container_width=True)
...
# Page: 5. Visualize Landscape
    ...
    for task_id, row in st.session_state.project_scores_df.iterrows():
        ...
        ax_quadrant.annotate(task_id.split(':')[0],  # Display short ID
                             (row['efficiency_impact'] +
                              0.05, row['risk_level'] + 0.1),
                             fontsize=9, weight='bold')

    ax_quadrant.axhline(y=2.5, color='gray', linestyle='--',
                        alpha=0.6)  # Mid-point for Risk
    ax_quadrant.axvline(x=3.0, color='gray', linestyle='--',
                        alpha=0.6)  # Mid-point for Efficiency Impact

    ax_quadrant.text(4.5, 1.2, 'Quick Wins', fontsize=12,
                     color='green', fontweight='bold', ha='center', va='center')
    ax_quadrant.text(1.5, 1.2, 'Low Impact / Low Risk', fontsize=12,
                     color='gray', fontweight='bold', ha='center', va='center')
    ax_quadrant.text(1.5, 4.0, 'CAUTION (High Risk)', fontsize=12,
                     color='red', fontweight='bold', ha='center', va='center')
    ax_quadrant.text(4.5, 4.0, 'Strategic / High Risk', fontsize=12,
    ...
    ax_quadrant.legend(unique_handles, unique_labels, title="Automation Tier",
    ...
    st.markdown(
        f"Projects ranked by their 'Efficiency-First' suitability score.")
    fig_bar, ax_bar = plt.subplots(
        figsize=(12, max(8, len(st.session_state.project_scores_df) * 0.7)))
    ranked_projects_eff = st.session_state.project_scores_df.sort_values(
        'suit_efficiency', ascending=True)
    ...
    st.markdown(
        f"Select 2-3 projects to compare their score profiles across dimensions.")
    ...
    selected_tasks_for_radar = st.multiselect(
        ...
        # Default to first 3 if available
        default=all_projects[:min(3, len(all_projects))],
    ...
    if len(selected_tasks_for_radar) > 0:
        ...
        fig_radar, axes_radar = plt.subplots(1, len(selected_tasks_for_radar), figsize=(
            5 * len(selected_tasks_for_radar), 6), subplot_kw=dict(polar=True))
        ...
        for i, task_id in enumerate(selected_tasks_for_radar):
            values = st.session_state.project_scores_df.loc[task_id,
                                                            st.session_state.score_columns].tolist()
            ...
            ax.set_xticklabels(st.session_state.score_columns,
                               fontsize=9, rotation=45, ha='right')
            ax.set_rlim(0, 5)  # Radial limits up to 5

            ax.plot(angles, values, linewidth=2, linestyle='solid', label=task_id,
                    color=st.session_state.project_scores_df.loc[task_id, 'color'])
            ax.fill(
                angles, values, color=st.session_state.project_scores_df.loc[task_id, 'color'], alpha=0.25)
            ax.set_title(
                f"Profile: {task_id.split(':')[0]} ({st.session_state.project_scores_df.loc[task_id, 'tier']})", va='bottom', fontsize=12)
...
# Page: 6. Deep Dive & ROI
    ...
    st.markdown(r"""
$$
ROI = \frac{\text{Annual Hours Saved} \cdot \text{Cost\_per\_Hour}}{\text{Development\_Cost} + \text{Annual\_Operating\_Cost}} - 1
$$
""")
    ...
    st.dataframe(st.session_state.hybrid_routing_df,
    ...
    if total_manual_time > 0:
        time_reduction_percent = (
            1 - (total_hybrid_time / total_manual_time)) * 100
    else:
        time_reduction_percent = 0
    st.markdown(
        f"Total manual time for T4: **{total_manual_time:.2f} minutes**")
    st.markdown(
        f"Total hybrid-routed time for T4: **{total_hybrid_time:.2f} minutes**")
    ...
    sorted_projects = st.session_state.project_scores_df.sort_values(
        'suit_efficiency', ascending=False)
    ...
    if not sorted_projects.empty:
        ...
        st.session_state.selected_top_project_id = st.selectbox(
            ...
            index=sorted_projects.index.tolist().index(
                sorted_projects.index[0]),  # Default to top-ranked
            key="roi_project_selector"
        )

        top_project_data = st.session_state.project_scores_df.loc[
            st.session_state.selected_top_project_id]

        st.markdown(
            f"**Selected Project:** '{st.session_state.selected_top_project_id}'")
        st.markdown(f"**Automation Tier:** {top_project_data['tier']}")
        st.markdown(
            f"**Efficiency-First Suitability Score:** {top_project_data['suit_efficiency']:.2f}")
        ...
        # Calculate ROI using the widget keys
        if (st.session_state.roi_dev_cost + st.session_state.roi_op_cost) > 0:
            roi_calculated = (st.session_state.roi_hours * st.session_state.roi_cost_per_hour) / \
                             (st.session_state.roi_dev_cost +
                              st.session_state.roi_op_cost) - 1
        else:
            roi_calculated = 0  # Prevent division by zero

        st.markdown(f"---")
        st.markdown(
            f"#### Business Case Summary for '{st.session_state.selected_top_project_id}'")
        st.markdown(
            f"Estimated Annual Savings: **${st.session_state.roi_hours * st.session_state.roi_cost_per_hour:,.2f}**")
        st.markdown(f"Calculated ROI: **{roi_calculated:.2%}**")

        st.markdown(f"---")
        st.markdown(f"#### Conceptual Timeline & Risk Mitigation:")
        st.markdown(
            f"  Timeline: Estimated 4-6 weeks for development and initial rollout.")
        st.markdown(
            f"  Risk Mitigation: Implement robust data validation checks, phased rollout, and human-in-the-loop review for exceptions.")
    else:
        st.info(
            "No projects available for ROI analysis. Please add projects on the 'Score Projects' page.")
...
# Page: 7. Automation Roadmap
    ...
    final_roadmap = st.session_state.project_scores_df.sort_values(
        'suit_efficiency', ascending=False)

    st.dataframe(final_roadmap[['timeline', 'hours_saved_month', 'tier',
                 'suit_efficiency', 'suit_risk']].round(2), use_container_width=True)
    ...
    st.markdown(
        f"1. A clear ranking of projects based on weighted suitability, allowing for strategic alignment.")
    st.markdown(
        f"2. Classification into appropriate automation tiers, guiding technology selection.")
    st.markdown(
        f"3. Visualizations that facilitate stakeholder communication and highlight key trade-offs.")
    st.markdown(
        f"4. A conceptual ROI for top projects, enabling data-driven investment decisions.")
